...
void rick_shc2d(double *cslm,double *dslm,int lmax,int ivec,
		double *rdatax,double *rdatay,
		struct rick_module *rick)
{
  ...
  //
  // compute the Plm first
  rick_compute_allplm(lmax,ivec,plm,dplm);
  //
  // call the precomputed subroutine
  rick_shc2d_pre(cslm,dslm,lmax,plm,dplm,ivec,rdatax,rdatay);
...
// */
void rick_shc2d_pre(double cslm,double dslm,int lmax,
		    double *plm, double *dplm,int ivec,
		    float *rdatax,float *rdatay, 
		    struct rick_module *rick)
{
  /* //
  ...
  double  dpdt,dpdp;
  int  i,j,m,m2,j2,ios1,l,lmaxp1,lmaxp1t2,oplm,nlon2;
  if(!rick->initialized){
  ...
  hc_vecalloc(&valuex,nlon2,"rick_shc2d_pre 1");
  if(ivec)
    hc_vecalloc(&valuey,nlon2,"rick_shc2d_pre 2");
  
  for (i=0;i < nlat; i++) {	
    /*  
	loop through latitudes (changed indices TWB) 
    */
	...
	valuex[m2]   +=  /* cos term */
	  plm[oplm+j] * cslm[j2]; /* A coeff */
	valuex[m2+1] +=  /* sin term */
	  plm[oplm+j] * cslm[j2+1];   /* B coeff */
      }

      /* compute inverse FFT  */
      rick_cs2ab((valuex+1),rick->nlon);	/* Note: cs2ab index changed, need valuex+1 */
      /* inverse FFT */
      rick_realft((valuex+1),rick->nlat,-1);	/* Note: realft index changed, pass as +1 */
	...
	/* derivative factors */
	lm1 = l - 1;
	dpdt = rick->dplm[oplm+j] * rick->ell_factor[lm1]; /* d_theta(P_lm) factor */
	dpdp = ((double)m) * rick->plm[oplm+j]/
	  rick->sin_theta[i] * rick->ell_factor[lm1]; /* d_phi (P_lm) factor */
	/* add up contributions from all l,m 
	...
	*/
	...
	valuey[m2+1] +=   // sin term
	  - cslm[j2]   * dpdt  - dslm[j2+1] * dpdp;
        ...
        /* do inverse FFTs */
      rick_cs2ab(valuex+1,rick->nlon);	/* Changed indices in cs2ab and realft */ 	 	
      rick_cs2ab(valuey+1,rick->nlon); 	/* to c-standard (ie. 0->n), so need value+1 here */
      rick_realft(valuex+1,rick->nlat,-1);
      rick_realft(valuey+1,rick->nlat,-1);
      /* assign to output array */
      for (j=0; j < rick->nlon; j++) {            /*  can't vectorize, since there's an offset of 2 */
	rdatax[ios1 + j] = valuex[j]/(double)(rick->nlat);
	rdatay[ios1 + j] = valuey[j]/(double)(rick->nlat);
      }
    }
  } /* end latitude loop */
...
void rick_shd2c(double *rdatax,double *rdatay,int lmax,int ivec,
		double *cslm,double *dslm,struct rick_module *rick)
{
  //
  ...
  rick_shd2c_pre(rdatax,rdatay,lmax,plm,dplm,ivec,cslm,dslm);
...
//
rick_shd2c_pre(double *rdatax,double *rdatay,int lmax,
	       double *plm,double *dplm,int ivec,
	       double *cslm,double *dslm, 
	       struct rick_module *rick)
{
  // local
  double *valuex, *valuey;
  ...
  /* allocate */
  hc_vecalloc(&valuex,nlon2,"rick_shd2c_pre 1");
  if(ivec)
    hc_vecalloc(&valuey,nlon2,"rick_shd2c_pre 2");
  ...
  for(i=0;i < rick->nlat;i++){
	...
	valuex[j] = rdatax[ios1 + j];
      ...
      // compute the FFT 
      //
      /* THESE TWO ROUTINES TAKE -1 POINTERS! */
      rick_realft((valuex+1),rick->nlat,+1);
      rick_ab2cs((valuex+1),rick->nlon);
	...
	if (m == 0) {
	  dfact = (rick->gauss_w[i] * plm[oplm+j])/2.0;
	}else{
	  dfact = (rick->gauss_w[i] * plm[oplm+j])/4.0;
	}
	m2 = m * 2;
	...
	dpdt = dplm[oplm+j];
	// d_phi (P_lm) factor
	dpdp = ((double)m) * plm[oplm+j]/rick->sin_theta[i];
	//
	m2 = m * 2;
	       ...
	       int *tnplm, struct rick_module *rick)
{
  ...
  static rick_boolean was_called = FALSE;
  ...
  if(!was_called){
    ...
    if(fabs((int)(xtemp+.5)-xtemp) > 1e-7){
      ...
      fprintf(stderr,"rick_init: maybe use lmax = ',2**(int(xtemp)+1)-1\n");
    ...
    *npoints = rick->nlat * rick->nlon; 
    ...
    //
    rick->dphi = RICK_TWOPI / (double)(rick->nlon);
    ...
    *tnplm = *nplm * (1+ivec);           // for all layers
    ...
    // evaluated
    //
    hc_dvecalloc(&rick->gauss_z,rick->nlat,"rick_init 1");
    hc_dvecalloc(&rick->gauss_w,rick->nlat,"rick_init 2");
    hc_dvecalloc(&rick->gauss_theta,rick->nlat,"rick_init 3");
    /* 
       gauss weighting 
    */
    ...
    if(ivec){
      ...
      //
      hc_dvecalloc(&rick->ell_factor,rick->nlat,"rick init 9");
      hc_dvecalloc(&rick->sin_theta,rick->nlat,"rick init 9");
      
      // 1/(l(l+1)) factors
      for(i=0,l=1;i < rick->nlat;i++,l++){
	// no l=0 term, obviously
	// go from l=1 to l=lmax+1
	rick->ell_factor[i] = 1.0/sqrt((double)(l*(l+1)));
...
compute legendre functions at all latitude levels by calling 
plmbar1
...
{
  //input: lmax,ivec
  // input/output: plm,dplm
  int i,os;
  if(lmax != rick->nlat-1){
    fprintf(stderr,"rick_compute_allplm: error: lmax mismatch: nlat: %i lmax: %i\n",
	    nlat,lmax);
    exit(-1);
  }
  os = 0;
  for(i=0;i < rick->nlat;i++){
    rick_plmbar1((plm+os),(dplm+os),ivec,lmax,rick->gauss_z[i]);
    os += lmsize;
  }
}
		  ...
		  double z, struct rick_module *rick)
{
  ...
  //
  int i,l,m,k,kstart,isize,l2,lstart,mstop,lmaxm1;
